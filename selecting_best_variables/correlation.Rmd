...
{
  #x="x1"; y="y1"
  p=ggplot(anscombe_data, aes_string(x,y))  + geom_smooth(method='lm', fill=NA) + geom_point(aes(colour=factor(1), fill = factor(1)), shape=21, size = 2) + ylim(2, 13)+ xlim(4, 16) + theme_minimal() + theme(legend.position="none") +  annotate("text", x = 12, y =4.5, label = sprintf("%s: %s", type, round(value,2)))  
  return(p)
}
...
mic_1=mine(anscombe_data$x1, anscombe_data$y1)$MIC
mic_2=mine(anscombe_data$x2, anscombe_data$y2)$MIC
mic_3=mine(anscombe_data$x3, anscombe_data$y3)$MIC
mic_4=mine(anscombe_data$x4, anscombe_data$y4)$MIC
...
## calculating the MIC for every pair
mic_r2_1=mine(anscombe_data$x1, anscombe_data$y1, alpha = 1)$`MIC-R2`
mic_r2_2=mine(anscombe_data$x2, anscombe_data$y2, alpha = 1)$`MIC-R2`
mic_r2_3=mine(anscombe_data$x3, anscombe_data$y3, alpha = 1)$`MIC-R2`
mic_r2_4=mine(anscombe_data$x4, anscombe_data$y4, alpha = 1)$`MIC-R2`
...
heart_disease_3 = data.frame(predict(dmy, newdata = heart_disease))

## please dont omit NA without an impact analysis first
...
We'll use `corrplot` package in R which can plot a `cor` object (classical correlation matrix), or any other matrix. We will plot **MIC** matrix in this case, but any other can be used as well, for example **MAS**.
...
par(omi=c(0,0,0.2,0))        ## (bottom, left, top, right)  in inches  

## Correlation plot with circles
corrplot(mine_res_hd$MIC, method="circle",col=brewer.pal(n=10, name="PuOr"),
         type="upper", # only display upper diagonal
         order="hclust", # order the matrix based on its correlation
         tl.col="red", tl.cex = 0.9, tl.srt=45, #label color, size and rotation
...
corrplot(mine_res_hd$MIC, method="color",
         type="upper", order="hclust", number.cex=0.6,
...
### Calculating correlation against one variable

```{r}

#####
mine_res_hd=mine(heart_disease_4, master = ncol(heart_disease_4))

a=data.frame(a=mine_res_hd$MIC)

arrange(data.frame(a=mine_res_hd$MIC), -has_heart_disease.yes
)
