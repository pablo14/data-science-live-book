...
This chapter contains both methodological and practical aspects on measuring correlation in variables. We will see that correlation word can be translated into "**functional relationship**".

In methodological you will find an the Anscombe Quartet, a set of four plots with disimilar spatial distribution, but sharing the same linear regression or correlation measure. We'll go one step ahead re-calculating their relationship though a more robust metric (MIC).

We will mention several times **Information Theory**, altough by now it's not going to be covered at mathematical level. But it's planed to.

Understanding these concepts in low dimension (two variables) and small data (a bunch of rows) allow us to better understand high dimensional data, nonetheless some real cases work data are _small_ data.

From the pactical point of view, you'll be able to replicate the analysis with your own data, profiling and exposing their relationships in fancy plots.
...
Perhaps the most classical  correlation measure for numeric variables is the `R statistic` (or Pearson coefficient) which goes from `1` _positive correlation_ to `-1` _negative correlation_. A value around `0` implies no correlation.

Squaring this number returns the `R squared` statistic (aka `R2`), which goes from `0` _no correlation_ to `1` _high correlation_. 

Function `correlation_table` retrieves R  metric for all numeric variables skipping the categorical ones.

```{r}
correlation_table(data=heart_disease, str_target="has_heart_disease")
```

Last function is a wrapper for function `cor`
...
4-different plots, having the same `mean` for every `x` and `y` (9 and 7.501 respectively), and the same degree of correlation. You can check all the measures by typing `summary(anscombe_data)`. 

This is why is so important to plot relationships when analyzing correlations.

We'll back on this data further, it can be improved! First we'll introduce some concepts of information theory.
...
Let's plot a non-linear relationship, directly based from a function (negative exponential), and print the MIC value.
...
**About last example:** Generate data based on a function is only for teaching purposes. But the concept of noise in variables is quite common in _almost_ **every data set**, no matter its source. You don't have to do anything to add noise to variables, it's already there.
...
This point is important since the **MIC behaves like R2 does in linear relationships**, plus it adapts quite well to **non-linear** relationships as we saw before, retrieving an specific score metric (`MICR2`) to profile the relationship. 

<br>

### Measuring infromation on Anscombe Quartet

Remember the example we review at the begining? Every pair of Anscombe Quartet returns an **R2 of 0.86**. But based on its plots it was clearly that not every pair exhibits neither a good correlation nor a similar distribution of `x` and `y`.
...
As you may notice we increased the `alpha` value to 0.8, this is a good practice -according to the documentation- when we analyzed small samples. Default value is 0.6 and its maximun 1.

In this case, MIC value spotted the most spourious relationship in the pair `x4 - y4`. Probably due to the small amount of cases per plot (11 rows) the MIC was the same for all the others pairs. Having more cases will show different MIC values.
...
Last image shows two different approache to compare time series, the **euclidean** is more similar to MIC measure. While DTW can track similarities ocurring in different times.
...
MINE -and many other algorithms- only works with numerical data. We need to do a **data preparation** trick, converting every categorical variable into flag (or dummy variable).
...
If we use package `caret` from R, this conversion only takes 2 lines of code:
...
We'll use `corrplot` package in R which can plot a `cor` object (classical correlation matrix), or any other matrix. We will plot **MIC** matrix in this case, but any other can be used as well, for example **MAS** or other metric that returns an squared matrix of correlations.

The two plots are based in the same data but display the correlation in different ways. 

```{r}
library(corrplot) ## library wto plot that matrix
library(RColorBrewer) # to use the color pallete brewer.pal

# hack to visualize the maximun value of the scale excluding the diagonal (var against itself)
...
#### A comment about these kind of plots

They are useful only when the amount of variables is not big. Or if you perform a variable selection first, keeping in mind that every variable should be numerical. 

If there is some categorical variable in the selection you can convert it into numerical first and inspect the relationship between the variables, thus sneak peak how certain values in categorical variables are more related to certain outcome, like in this case.

<br>

#### How about some insights from the plots?

Since the variable to predict is `has_heart_disease`, it appears something interesing, to have a heart disease is more correlated to `thal=3` than to value `thal=6`.
...
Like a photographer shooting from different angules, or different cameras. The object is always the same, but the perspective gives different information.
...
If it last too much time to finish, consider taking a data sample.
Also it can be run in parallel, just setting `n.cores` parameter.
...
This time we'll use `infotheo` package, we need to first do a **data preparation** step, applying a `discretize` function (or binning) function present in package. It converts every numerical variable into categorical based on equal frequency criteria. 
...
A high MIC value indicates that the relationship between the two variables can be explain through a function. Is
our job to find that function or predictive model.

This anaylsis is extended to n-variables, this book introduces other algorithm in the selecting best variables chapter. 
...
More to come regarding **Information Theory**. By now you check these recommended lectures: 
...
We've started comparing classical R squared to a more robust correlation metrics based on information theory:
...
Regarding **data preparation** stage it is useful to profile variables according to the information present in the relationships, and other properties such their symetry. 
