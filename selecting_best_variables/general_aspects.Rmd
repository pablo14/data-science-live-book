...
This chapter covers key aspects with examples in R behind:

* Best variables ranking from conventional machine learning algorithms
* The nature in getting the variable selection (based and not on predictive models)
* Advice to explore the best subset in practice
* Feature engineering and clustering models
* The effect of variables working in groups (intuition and information theory)


_Selecting best variables is also known as feature engineering, selecting best predictors, selecting most important predictors, feature selection, among others._

<br>

### The intuition

Selecting best variables is like do a summary of a story, we want to focus on those few details that best describe what we're talking about. The balance is not to talk _too much_ about unnecessary details (overfitting), and not talk _too little_ about the essence of the story (underfitting).

Another example can be the decision of buying a new laptop: _what are the features that we care the most? Price, color and shipping method? Color and battery life? Or just price?_
...
To start from this perspective is important, since in the exploration of many algorithms that _rank_ the variables according to their predictive power we can found different -and similar- results. That is
...
Let's imagine based on algorithm 1, accuracy is 80%, while the accuracy based on algorithm 2 is 78%. Considering that every model has its inner variance, the result can be seen as the same. 
...
**Conclusion**: if results are not good the focus should be on improving and checking the **data preparation** step. _Next section will exemplify it._ 

<br>

### Going deeper into variable ranking

It's quite common to find in literature and algorithms of selecting best variables an univariate analysis report on them, that is a ranking of variables given particular metric.
...
Now we can proceed with the comparison. Columns: `importance_rf` and `importance_gbm` represent the importance measured by each algorithm. Based on each metric, there are `rank_rf` and `rank_gbm` which represent the importance order, finally `rank_diff` (`rank_rf` - `rank_gbm`) represents how different each algorithm rank.
...
Different predictive models implementations have their criteria to report what are the best features, according to that particular model. This ends up in different ranking across different algorithms. _More info about the inner importance metrics  can be found at <a href=https://topepo.github.io/caret/variable-importance.html" target="blank">caret documentation</a>._

Even more, in tree based model like GBM and Random Forest there is a random component to picking up variables, and the importance is based on prior -and automatic- variable selection when building the trees. The importance of each variable depends on the others, not only on its isolated contribution: **Variables work in groups**. We'll back on this later on this chapter.
...
This is interesting since they are no as popular as the other ones, but they are proved to perform really well in areas related to genomic data. They need to find those _relevant_ genes (input variable) that are correlated with some disease, like cancer (target variable).

This area differs from others most standard in the number of variables to analyze (in the order of thousands), really huge in comparison to others. 
...
This point is not related to algorithmic procedures, but to the area from which the data comes.

Imagine data coming from a survey. This survey has one year of history, and during the first three months there wasn't a good control when inserting data, so users can type whatever they want. Variables during this period will probably be spurious. 

It's easy to recognize when during a given period data, it'is empty or null. Also when there are extreme values. 

We should ask a question: 

_Is it reliable this data?_ Keep in mind the predictive model will learn _as a kid_, it will not judge the data just learn from it. If data is spurious in a given period, then remove these input cases.
...
For example: Building a model based on variable `var_1` could lead to an overall accuracy of 60%. On the other hand, build a model based on `var_2` could reach an accuracy of 72%. But when we combine these two `var_1` and `var_2` variables, we could achieve an accuracy above 80%.
...
Following code illustrates what Aristotle said _some_ years ago. 
...
When the same input is related to different outcomes is defined as **noise**. The intuition is the same as one person tell us: _"Hey it's going to rain tomorrow!"_, and another one says: _"For sure tomorrow is not going to rain"_.  
...
Nonetheless, it's a deeper topic to cover, considering last analysis, how about talking an `Id` column (every value is unique) to predict something? The correspondence between input-output will also be unique... but is it a useful model? There'll be more to come about information theory in this book.

<br>

#### Conclusions  

* The proposed R example based on `heart_disease` data shows an average **improvement of 9%** when considering two variables at a time, not bad. This percentage of improvement is the result of the variables working in groups.
...
* Also the **work in groups** effect is higher if input variables **are not correlated between** them. This is difficult of optimizing in practice. More on this on next section...
