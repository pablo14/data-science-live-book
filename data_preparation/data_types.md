...
To be sure, we can delegate some tasks to automatic processes; however, humans are indispensable to monitor and adjust the model, givin the correct input data.
...
The conversion introduces bias to the analysis. A similar case exists when we deal with missing values: <a href="http://livebook.datascienceheroes.com/data_preparation/treating_missing_data.html" target="blank">Handling and Imputation of Missing Data</a>.
