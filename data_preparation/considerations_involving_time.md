...
As time goes by, variables may change its values as well, making the time-analysis crucial in order to create a predictive model. Avoiding the use of **effects** as **causes**.
...
* How to analyze and prepare variables that increase -or decrease- its value to infinity (and beyond).
...
The key concept here: **how to handle time in predictive modeling**. It's a good opportunity to ask: _How would it be possible to addresshis time issues with automatic systems?_ 
