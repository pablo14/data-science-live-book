...
_Note: Other name to reduce cardinality is: **encoding**._
...
# exploring data
freq(select(data_country, -person))
```
...
We've a complete profiling of `country` regarding the target variable `has_flu`. Each row represent an unique category of `input` variable, and each row an attribute that defines each category in terms of representative and likelihood. 
...
Despite the fact this is a prepared example, there are some data preparations techniques that can be really useful in terms of accuracy but they need some supervision. This supervision can be controlled helped by algorithms.
...
### Re-grouping having a numerical or multi-nominal variable
...
* We saw two cases to reduce cardinality, the first one doesn't care about target variable, which can be dangerous in predictive model, while the second does the reduction by grouping categories which share similar behavior.
...
* What was mention at the beginning respect to **destroy the information in the input variable**, implies that the resultant grouping have the same rates across groups (in a binary variable input). [2]
