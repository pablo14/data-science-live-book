...
```{r, fig.height=4, fig.width=6,results=FALSE}
...
MICE stands for "Multivariate Imputation by Chained Equations", also known as: Fully Conditional Specification.
...
Internally it fill variable `A`, based on `B` and `C`. Then it fill `B` based on `A` and `C` (`A` previously predicted). And the iteration continues...
...
imputation_res$OOBerror

# Final imputed data frame
imputation_res$ximp

df_after=imputation_res$ximp
...
df_after$is_imputed=T

df_holly$is_imputed=F

df_all=rbind(df_holly, df_after)


ggplot(df_all, aes(TheatersOpenWeek, colour = is_imputed)) + geom_density()
```

I
They look pretty similar!

f we want to analyze from the analytical point of view, and don't play to be an art critic looking at a painting, we may run an statistical test comparing, for example, the means or the variance.
...
The Imputation methods when the missing values, irea ral missing value, a **controlled-bias**, to we have not to exclude the variables from the analysis nor the predictive model.
