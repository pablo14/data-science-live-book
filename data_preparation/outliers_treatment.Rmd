...
Outliers can be other name for **anomaly detection**, and its treatment in time series can be slighty different. Where each point, a value for certain date, is correlated with the another previous values.
...
_This last analysis is like imaging a picture by what another person tell us: we convert the voice (which is a signal) into an image in our brain._ `r emo::ji("speaking_head")` `r emo::ji("roll_eyes")` ... => `r emo::ji("mountain_snow")` 
