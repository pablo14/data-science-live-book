...
```{r equal-frequency-discretization, fig.width=4, fig.height=3, tidy=FALSE, fig.cap="", out.extra='Equal frequency example'}
...
```{r, binning-variable, tidy=FALSE, fig.cap="", out.extra='Automatic discretization results'}
...
```{r data-preparation-profiling-nominal-variable_2, fig.height=2.3, fig.width=4.2}
...
```{r outliers-intuition, echo=FALSE ,out.width="100%", fig.cap="Outliers", out.extra=''}
...
```{r dealing-with-outliers-2, fig.height=3, fig.width=4, echo=FALSE}
...
```{r dealing-with-outliers-3, fig.height=3, fig.width=4, echo=FALSE,tidy=TRUE}
# Plotting the same distribution plus the percentiles
ggplot(df_1, aes(var)) + geom_histogram(bins=20) +
...
```{r Finding-the-cutpoint, echo=FALSE, out.width="30%"}
...
```{r numerical-outliers-1, echo=FALSE}
...
```{r categorical-outliers-1, echo=FALSE, message=FALSE, warning=FALSE}
library(funModeling)
library(dplyr)
...
```{r outlier-analysis-in-r, echo=FALSE}
freq(df_hello_world)
```
...
```{r boxplot-in-r, echo=FALSE, out.width="80%"}
...
```{r, outliers-method-comparison, warning=FALSE, message=FALSE}
# First we need to convert the dataset into wide format
...
```{r na-missing-values-analysis, fig.height=3, fig.width=6, echo=FALSE}
...
HollywoodMovies2011$TheatersOpenWeek_3=
...
```{r, fig.height=3, fig.width=6, warning=FALSE, message=FALSE, tidy=FALSE}
# Filling all NA values with the mean of the variable
HollywoodMovies2011$opweek_mean=
...
p2=ggplot(HollywoodMovies2011, aes(x=opweek_mean)) + 
  geom_histogram(colour="black", fill="white") + 
  ylim(0, 30)
...
df_holly$TheatersOpenWeek_3=
  prodNA(select(df_holly, TheatersOpenWeek_3), 0.15)[,1]

# missForest fails if it has any character variable, 
# so we convert the only character into the factor:
df_holly$TheatersOpenWeek_2 = 
  as.factor(df_holly$TheatersOpenWeek_2)
...
```{r how-to-impute-missing-values, warning=FALSE, message=FALSE, fig.height=4, fig.width=5}
...
```{r missing-value-imputation, fig.height=4, fig.width=7, tidy=FALSE}
# An ugly hack to plot NA as a category
levels(df_all$TheatersOpenWeek_3)=
  c(levels(df_all$TheatersOpenWeek_3), "NA")

flag_na=is.na(df_all$TheatersOpenWeek_3)

df_all$TheatersOpenWeek_3[flag_na]="NA"

# Now the plot!
ggplot(df_all, aes(x = TheatersOpenWeek_3, 
                   fill = TheatersOpenWeek_3)
       ) +
  geom_bar(na.rm=T) + 
...
```{r data-preparation-time-variable,echo=FALSE, fig.height=3, fig.width=4}
library(ggplot2)
...
```{r data-preparation-time-variable-1, echo=FALSE, fig.height=3, fig.width=4}
...
```{r data-preparation-time-variable-3, echo=FALSE, fig.height=3, fig.width=4}
...
```{r data-preparation-time-variable-4, echo=FALSE, fig.height=3, fig.width=4}
