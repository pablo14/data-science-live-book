...
* **Model too complicated**: Let's say we have lots of input variables, which casuses **high variance**. The model will overfit on training data, having a low accuracy on test data.
* **Model too simple**: On the other hand, the model may not be capturing all the information from the data.
* **Not enough input data**: Data forms shapes in a n-dimensional space (where `n` is all the input+target variables). If there are not enough points, the surface is not developed well enough.
