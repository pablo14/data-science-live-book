...
We looked at the 80% of the population, which seems a good number to describe where most of the cases are. We could also have used the 90% (pecentile 95th - 0.5th). 
...
q_25=quantile(data_poverty$newest_value, probs = 0.25, na.rm=T)
quantile

## Also we can get multiple quantiles at once:
q_quartile=quantile(data_poverty$newest_value, probs = c(0.25, 0.5, 0.75), na.rm=T)
q_quartile
```
...
```{r, warning=FALSE}
df_q=data.frame(value=q_quartile, quantile=c("25th", "50th", "75th"))

library(ggplot2)
ggplot(data_poverty, aes(newest_value)) + geom_histogram() +
  geom_vline(data=df_q, 
             aes(xintercept=value, 
                 colour = quantile),
             show_guide = TRUE, linetype="dashed") + theme_light()


    ggplot(data_poverty, aes(newest_value)) + geom_histogram() + geom_vline(aes(xintercept=mean(data_poverty$newest_value, na.rm=T), color="mean"), linetype="dashed", size=1) + geom_vline(aes(xintercept=, color="mean"), linetype="dashed", size=1) 
```
...
```{r}
## Also we can get multiple quantiles at once:
q_01_10_90_99=quantile(data_poverty$newest_value, probs = c(0.01, 0.1, 0.9, 0.99), na.rm=T)
q_01_10_90_99
```
...
Percentile and ranking cases consider each value in terms of the others.
